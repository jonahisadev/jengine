...
namespace JEngine {
    ...
    : TexturedQuad(x, y, width, height, path), _cell_size(cell_size), _grid_size(float(getImageWidth()) / cell_size)
    {
        setGridPosition(0, 0);
    }
    ...
    void Spritesheet::updateUV() {
        ...
        float uv[] = {
            ...
            coords.u() + (_cell_size / (float)getImageWidth()), coords.v() + (_cell_size / (float)getImageHeight()),
